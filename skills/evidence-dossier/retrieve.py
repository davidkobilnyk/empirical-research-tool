...
# The identity helpers live either beside this module (when shipped inside a
# skill) or in the repo's skill copy (when running from a checkout). Look
# locally first so the pipeline is relocatable: a module that can only find its
# dependency at one absolute path is not shippable.
SKILL_KERNEL = os.path.join(REPO_ROOT, "skills", "doi-source-identity", "kernel.py")
LOCAL_IDENTITY = "identity.py"


def identity_path():
    here = os.path.dirname(os.path.abspath(__file__))
    local = os.path.join(here, LOCAL_IDENTITY)
    return local if os.path.exists(local) else SKILL_KERNEL


def runs_root():
    """Where run directories go.

    The repo's runs/ when running from a checkout, otherwise ./runs under the
    current working directory. A skill runs in a session workspace with no repo,
    so an absolute repo path cannot be the default.
    """
    repo_runs = os.path.join(REPO_ROOT, "runs")
    root = (repo_runs if os.path.isdir(os.path.join(REPO_ROOT, ".git"))
...
def load_identity():
    ...
    spec = importlib.util.spec_from_file_location("doi_source_identity", identity_path())
...
def run_live(question, mcp, servers, disconfirming=None, limits=None, email=None,
    ...
    outdir = outdir or os.path.join(runs_root(), 
...
def run_replay(payload_path, question="(replayed payloads)", outdir=None,
    ...
    note = coverage_note(question, queries, stats, integrity, sorted(per_set), unavailable)
    outdir = outdir or os.path.join(runs_root(),  "replay-" +
